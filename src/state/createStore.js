...
const defaultState = { count: 0, total: 0, shipping: 0, grandTotal: 0, items: [] }

const save = (state) => windowGlobal && localStorage.setItem('cart', JSON.stringify(state))
const load = () => windowGlobal && JSON.parse(localStorage.getItem('cart'))
const cartStateValid = (cartState) => cartState && cartState.items && cartState.count != undefined && cartState.grandTotal != undefined

const sum = (accumulator, value) => accumulator + value
const countFunc = (items = []) => items.map((item) => item.quantity).reduce(sum, 0)
