...
const CartItem = ({ item, add, remove }) => (
  <div className='itemRow'>
    ...
    <div className='item-systemname'>{item.content}l {item.name}</div>
...
const Cart = ({ checkout, count, shipping, total, grandTotal, items, add, remove, clear }) => (
  <div className={'cart ' + (checkout ? 'checkout' : '')} style={{ display: count > 0 ? 'initial' : 'none' }}>
    <div className='summary'>
      ...
      <div className='grand-total simpleCart_grandTotal'>€{grandTotal.toFixed(2)}</div>
    </div>
    ...
    <div className='details'>
      ...
      <div className='cart-items'>
        <div className='simpleCart_items'>
          <div>{items.map((item, index) => <CartItem item={item} key={index} add={add} remove={remove} />)}</div>
        </div>
      </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    add: (item, quantity = 1) => dispatch({ type: 'ADD', payload: { item, quantity } }),
    remove: (item, quantity = 1) => dispatch({ type: 'REMOVE', payload: { item, quantity } }),
